...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, dropout, dim, max_len=5000, pe_noise_size=0):
        if dim % 2 != 0:
        ...
        self.max_noise_size = pe_noise_size
        logger.info("Max Noise Size: {}".format(self.max_noise_size))
    ...
    def forward(self, emb, step=None):
        ...
        if self.training and step == 0 and self.max_noise_size > 0:
            seq_len, bsz, _ = emb.shape

            pe_size: int = min(self.pe.size(0) - emb.size(0), self.max_noise_size)

            positions = torch.arange(seq_len, device=self.pe.device).repeat(bsz, 1)
            position_offset = torch.randint(0, pe_size, size=(bsz,), device=self.pe.device)
            positions = positions + position_offset[:, None]

            pe_alt = self.pe.index_select(0, positions.view(-1)).view(bsz, seq_len, -1).transpose(0, 1).detach()

            emb = emb + pe_alt
        else:
            emb = emb + self.pe[step+self.max_noise_size:emb.size(0) + step + self.max_noise_size]
...
class Embeddings(nn.Module):
    ...
    def __init__(self, word_vec_size,
        ...
        if self.position_encoding:
            pe = PositionalEncoding(dropout, self.embedding_size, pe_noise_size=pe_noise_size)
