...
class Inference(object):
    ...
    def _gold_score(
        ...
        if "tgt" in batch.__dict__:
            gs, gold_ppl = self._score_target(
                batch,
                memory_bank,
                src_lengths,
                src_vocabs,
    ...
    def dump_feature(
        ...
        for n, batch in enumerate(data_iter):
            feature = self.dump_feature_batch(
                batch, data.src_vocabs, attn_debug
            )
            feature = [x.data.cpu().numpy() for x in feature]
            feature_matrix = np.concatenate(feature, axis=0)
            numpy.save(os.path.join(dest, str(n)), feature_matrix)
    ...
    def dump_feature_batch(self, batch, src_vocabs, attn_debug):
        """Translate a batch of sentences."""
        raise NotImplementedError
...
class Translator(Inference):
    ...
    def dump_feature_batch(self, batch, src_vocabs, attn_debug):
        ...
        with torch.no_grad():
            ...
            return self._dump_feature_with_strategy(
                batch, src_vocabs, decode_strategy
            )
    ...
    def _run_encoder_for_feature_dumping(self, batch):
        src, src_lengths = (
            batch.src if isinstance(batch.src, tuple) else (batch.src, None)
        )

        features = self.model.encoder.get_feature(
            src, src_lengths
        )
        return features
    ...
    def _translate_batch_with_strategy(
        ...
        gold_score, gold_ppl = self._gold_score(
            batch,
            memory_bank,
            src_lengths,
            src_vocabs,
    ...
    def _score_target(
        ...
        ppl_lists = log_probs.gather(2, gold)[:, :, 0].transpose(0, 1).cpu().numpy().tolist()
        index_lists = gold[:,:,0].transpose(0, 1).cpu().numpy().tolist()
        gold_ppls = []
        for ppl_list, index_list in zip(ppl_lists, index_lists):
            gold_ppl = []
            for ppl, index in zip(ppl_list, index_list):
                if index not in [self._tgt_pad_idx, self._tgt_eos_idx]:
                    gold_ppl.append(ppl)
            gold_ppls.append(gold_ppl)

        return gold_scores, gold_ppls

    def _dump_feature_with_strategy(
            self, batch, src_vocabs, decode_strategy
    ):
        """Translate a batch of sentences step by step using cache.
        ...
        # (1) Run the encoder on the src.
        features = self._run_encoder_for_feature_dumping(batch)
        return features
