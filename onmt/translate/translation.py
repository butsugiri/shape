...
class TranslationBuilder(object):
    ...
    def from_batch(self, translation_batch):
        ...
        preds, pred_score, attn, align, gold_score, gold_ppl, indices = list(zip(
            *sorted(zip(translation_batch["predictions"],
                        translation_batch["scores"],
                        translation_batch["attention"],
                        translation_batch["alignment"],
                        translation_batch["gold_score"],
                        translation_batch["gold_ppl"],
                        batch.indices.data),
                    key=lambda x: x[-1])))
        ...
        for b in range(batch_size):
            ...
            translation = Translation(
                ...
                gold_sent, gold_score[b], align[b], gold_ppl[b]
            )
...
class Translation(object):
    ...
    __slots__ = ["src", "src_raw", "pred_sents", "attns", "pred_scores",
                 "gold_sent", "gold_score", "word_aligns", 'gold_ppl']

    def __init__(self, src, src_raw, pred_sents,
                 attn, pred_scores, tgt_sent, gold_score, word_aligns, gold_ppl):
        self.src = src
    ...
    def log(self, sent_number):
        ...
        if self.gold_sent is not None:
            ...
            ppls = ' '.join(['{:.4f}'.format(x) for x in self.gold_ppl])
            msg.append(("GOLD PPL {}\t{}\n".format(sent_number, ppls)))
